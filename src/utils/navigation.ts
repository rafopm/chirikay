...
export const isActiveRoute = (currentPath: string, href: string): boolean => {
  ...
  export const getLinkClasses = (isActive: boolean, baseClasses: string): string => {
  ...
  export class MobileMenuController {
    ...
    closeOnResize(): void {
      if (window.innerWidth >= 1024 && !this.navbar?.classList.contains("hidden")) {
        this.toggle();
      }
    }
    ...
    closeOnOutsideClick(event: Event): void {
      ...
      if (!this.navbar?.contains(target) && !this.toggleButton?.contains(target)) {
        if (!this.navbar?.classList.contains("hidden")) {
          this.toggle();
        }
      }
